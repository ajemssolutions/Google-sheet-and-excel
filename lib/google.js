...
async function exchangeCode(auth, code) {
  ...
  const session = {
    ...
    refresh_token: data.refresh_token || (auth.get() || {}).refresh_token || '',
...
const refreshing = new Map();   // credential handle -> in-flight refresh
...
async function token(auth) {
  ...
  if (Date.now() > (g.expires_at || 0) - 60000) {
    // Concurrent callers on the same credential share one refresh.
    const k = auth.key || 'default';
    if (!refreshing.has(k)) {
      refreshing.set(k, refresh(auth).finally(() => refreshing.delete(k)));
    }
    return (await refreshing.get(k)).access_token;
  }
 ...
 */
function scopeStatus(auth) {
  const g = auth ? auth.get() : null;
  if (!g || !g.access_token) {
...
async function api(auth, url) {
  const t = await token(auth);
  let res;
  try {
    res = await fetch(url, {
...
async function listSpreadsheets(auth) {
  ...
  const data = await api(auth, DRIVE + '/files?' + p.toString());
  return data.files || [];
}

/** One cheap call used by the scheduler to decide whether to read the sheet. */
async function fileMeta(auth, spreadsheetId) {
  return api(auth, DRIVE + '/files/' + encodeURIComponent(spreadsheetId) +
             '?fields=id,name,modifiedTime');
}

// ── Sheets ────────────────────────────────────────────────

async function listTabs(auth, spreadsheetId) {
  const data = await api(auth, SHEETS + '/' + encodeURIComponent(spreadsheetId) +
...
async function readTab(auth, spreadsheetId, tabTitle, headerRow) {
