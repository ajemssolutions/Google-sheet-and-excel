...
function credentialProblems() {
  ...
  if (!/^https?:\/\/localhost(:\d+)?\/oauth\/callback$/.test(c.redirectUri)) {
    out.push('GOOGLE_REDIRECT_URI is "' + c.redirectUri +
             '". It must match the Authorised redirect URI in Google Cloud exactly.');
  }
...
async function exchangeCode(code) {
  ...
  const session = {
    ...
    refresh_token: data.refresh_token || (store.get().google || {}).refresh_token || '',
...
async function listSpreadsheets() {
  ...
  const data = await api(DRIVE + '/files?' + p.toString());
  return data.files || [];
}

/** One cheap call used by the scheduler to decide whether to read the sheet. */
async function fileMeta(spreadsheetId) {
  return api(DRIVE + '/files/' + encodeURIComponent(spreadsheetId) +
...
async function listTabs(spreadsheetId) {
  const data = await api(SHEETS + '/' + encodeURIComponent(spreadsheetId) +
...
async function readTab(spreadsheetId, tabTitle, headerRow) {
