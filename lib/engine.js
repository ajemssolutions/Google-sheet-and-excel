...
async function readSourceTab(tenant, task, tab) {
  if (isUpload(task)) return excel.readTab(task.uploadId, tab, 1);
  // A scheduled run has no browser behind it, so the task's own captured
  // credential is what reads the sheet.
  return google.readTab(store.taskGoogle(tenant, task.id), task.spreadsheetId, tab, 1);
}
 ...
 */
async function seedLinks(tenant, task, sel) {
  const links = store.getLinks(tenant, task.id);
  let existing;
  try {
    existing = await ajems.listResponses(tenant, sel.getUrl || sel.postUrl);
  } catch (e) {
 ...
 */
async function runTask(tenant, taskId, trigger, opts) {
  opts = opts || {};
  const task = store.getTask(tenant, taskId);
  ...
  try {
    ...
    const meta = isUpload(task) ? {} : await google.fileMeta(store.taskGoogle(tenant, task.id), task.spreadsheetId);
    const links = store.getLinks(tenant, task.id);
    ...
    for (const sel of selections) {
      ...
      if (opts.seedFirst) {
        const s = await seedLinks(tenant, task, sel);
      ...
      let rows;
      try {
        const r = await readSourceTab(tenant, task, sel.tab);
        rows = r.rows;
      ...
      await pool(rows, opts.concurrency || 4, async (row) => {
        ...
        try {
          if (existing) {
            ...
            await ajems.updateResponse(tenant, sel.postUrl, existing.responseId, payload);
            existing.hash = hash;
            tabResult.updated++; result.updated++;
            return;
          }

          const created = await ajems.createResponse(tenant, sel.postUrl, payload);
