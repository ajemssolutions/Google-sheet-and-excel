...
function conn(tenant) {
  const a = (store.getOrg(tenant) || {}).ajems || {};
  return { baseUrl: a.baseUrl || '', secretKey: a.secretKey || '' };
}

function headers(tenant, extra) {
  return Object.assign({
    'X-Json-Builder-Secret-Key': conn(tenant).secretKey,
    'Accept': 'application/json'
  }, extra || {});
}

async function request(tenant, method, url, body) {
  const target = withSlash(url);
  const opts = {
    method,
    headers: headers(tenant, body ? { 'Content-Type': 'application/json' } : null),
    redirect: 'manual',
...
const get   = (tenant, url)       => request(tenant, 'GET', url);
const post  = (tenant, url, body) => request(tenant, 'POST', url, body);
const patch = (tenant, url, body) => request(tenant, 'PATCH', url, body);
...
async function createApp(tenant, base, payload) {
  return post(tenant, base + 'apps/', payload);
}

async function createForm(tenant, base, appId, fields, meta) {
  return post(tenant, base + 'forms/', Object.assign({
    app: appId,
    app_id: appId,
    title: meta.title,
...
async function getForm(tenant, detailUrl) {
  return get(tenant, detailUrl);
}

async function listResponses(tenant, responsesUrl) {
  const data = await get(tenant, responsesUrl);
  if (Array.isArray(data)) return data;
  return data.responses || data.results || data.data || [];
}

async function createResponse(tenant, responsesUrl, payload) {
  return post(tenant, responsesUrl, payload);
}

async function updateResponse(tenant, responsesUrl, responseId, payload) {
  return patch(tenant, responsesUrl + responseId + '/', payload);
}
