...
function setOrgGoogle(tenant, google) {
  const o = org(tenant);
  o.google = google;
  save();
  return o;
...
function newSession(tenant) {
  ...
  all().sessions[id] = { tenant, at: Date.now(), seen: Date.now() };
  save();
  return id;
}
...
module.exports = {
  ...
  getOrg, org, orgTenants, setOrgAjems, setOrgGoogle,
  newSession, sessionTenant, endSession,
